...
#########################################
# 3. 模板-影像 Cross-Attention (输出 K-token prefix)
#########################################

class CrossAttentionAligner(nn.Module):
    """
    将视觉 patch token h_v 与模板语义 h_t 对齐成 K 个 prefix token (R_p).
    输出形状 [B, K, H]，作为 residual 路径的前缀注入 LLM.
    """
    def __init__(
        self,
        d_v: int,
        ...
        # K 个可学习 query token（每个样本都会加上 h_t_proj 作为条件）
        self.query_tokens = nn.Parameter(torch.randn(1, num_prefix_tokens, d_hidden) * 0.02)

    def forward(self, h_v, h_t):
        # h_v: [B, N_patches, d_v]
        # h_t: [B, d_t]
        B, N, _ = h_v.shape
        h_v_proj = self.proj_v(h_v)                 # [B, N, H]
        h_t_proj = self.proj_t(h_t).unsqueeze(1)    # [B, 1, H]

        # 变成 K 个 query
        queries = h_t_proj + self.query_tokens      # [B, K, H]
...
class RPGModel(nn.Module):
    ...
    # ---------------------------
    def _expand_prefix(self, prefix_vec: torch.Tensor):
        """
        用于 template 路径：
          prefix_vec: [B,H] -> [B,K,H]
        residual 路径不需要（aligner 已直接输出 K token）。
        """
        if prefix_vec.dim() == 2:
            prefix_vec = prefix_vec.unsqueeze(1)  # [B,1,H]
        if prefix_vec.size(1) == 1 and self.num_prefix_tokens > 1:
            prefix_vec = prefix_vec.repeat(1, self.num_prefix_tokens, 1)  # [B,K,H]
        return prefix_vec

    def _build_prefix(self, prefix_vec: torch.Tensor):
        """
        prefix_vec: [B,H] or [B,1,H] or [B,K,H]
        return:
          prefix_embeds [B,K,H]
          prefix_mask   [B,K]
        """
    ...
    def forward_template(
        ...
        # 3) 用 h_t 构造 prefix (K tokens)
        prefix_vec = self.template_proj(h_t)        # [B,H]
        prefix_vec = self._expand_prefix(prefix_vec)  # [B,K,H]
    ...
    def forward_residual(
        ...
        # 3) cross-attn -> K-token R_p
        prefix_vec = self.cross_aligner(h_v, h_t)  # [B,K,H]
        prefix_embeds, prefix_mask = self._build_prefix(prefix_vec)
        K = prefix_embeds.size(1)
    ...
    def forward_residual_implicit_cf(
        ...
        # 2) cross-attn with same h_t -> K-token prefix
        h_t = h_t.to(device=device, dtype=self.lm.dtype)
        prefix_vec_cf = self.cross_aligner(h_v_cf, h_t)  # [B,K,H]
        prefix_embeds_cf, prefix_mask_cf = self._build_prefix(prefix_vec_cf)
        K = prefix_embeds_cf.size(1)
